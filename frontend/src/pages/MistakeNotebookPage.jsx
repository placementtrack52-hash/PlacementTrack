...
const MistakeNotebookPage = () => {
  ...
  return (
    <PageShell
      ...
      actions={<Link to="/dashboard" className="rounded-full bg-white px-5 py-3 text-sm font-semibold text-ink dark:bg-black dark:text-white">Back to dashboard</Link>}
    >
      <div className="space-y-4">
        {progress.mistakes.length ? (
          progress.mistakes.map((mistake) => (
            <div key={mistake.id} className="rounded-[1.75rem] bg-white p-6 shadow-soft dark:bg-black">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                <span className="rounded-full bg-[#faf6f0] px-3 py-2 text-xs font-semibold text-ink dark:bg-black dark:text-white">Weakness: {mistake.weaknessLabel}</span>
                <span className="rounded-full bg-[#edf4ff] px-3 py-2 text-xs font-semibold text-ink dark:bg-black dark:text-white">{mistake.timeTakenSeconds}s spent</span>
          ...
          <div className="rounded-[1.75rem] bg-white p-6 text-sm text-slate shadow-soft dark:bg-black dark:text-slate-300">
